...
class LaplaceRandomVariable(RandomVariable):
    ...
    def cdf(self, x):
        if x <= self.location:
            return 0.5 * math.exp( self.scale *  (x - self.location))
        else:
            return 1 - 0.5 * math.exp( - self.scale * (x - self.location))

    def quantile(self, alpha):
        x = self.location + math.log(alpha * 2) / self.scale
        if x <= self.location:
            return x
        return self.location - math.log(2 - alpha * 2) / self.scale
